...
html_temp = """<div style="background-color:#000;padding:10px;border-radius:10px">
                <h1 style="color:#000;text-align:center">Loan Eligibility Prediction App</h1> 
                <h4 style="color:#000;text-align:center">Made for: Credit Team</h4> 
                """
...
def run_ml_app():
    ...
    applicant_income = right.number_input("Applicant Income", min_value = 1000)
    coApplicant_income = left.number_input("coApplicant Income", min_value = 10)
    loan_amount = right.number_input("Loan Amount", min_value = 100)
    loan_amount_term = left.number_input("Loan Amount Term", min_value = 30, max_value = 360)
...
def predict(gender, married, dependent, education, self_employed, applicant_income, coApplicant_income
    ...
    prediction = Logistic_Regression_Model.predict([[gen, mar, dependent, edu, sem, applicant_income,
                                                    coApplicant_income, loan_amount, loan_amount_term, credit_history, pro]])
